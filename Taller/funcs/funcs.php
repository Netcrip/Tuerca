...
//recibe los errores y lo agrega mediante un div con un estilo de bootstrap
    ...
    function validaIdToken($id, $token){
        ...
        $stmt = $mysqli->prepare("SELECT activacion FROM usuarios WHERE uid = ? AND token = ? LIMIT 1");
...
//lo unico que hace es un UPDATE al campo activacion y lo coloca como 1 mediante el id
        $stmt = $myslqi->prepare("UPDATE usuarios SET activacion=1 WHERE uid = ?");
    ...
    function login ($usuario, $password)
    {
        ...
        $stmt = $mysqli->prepare("SELECT uid, id_tipo, password FROM usuarios WHERE usuario = ? || correo = ? LIMIT 1");
